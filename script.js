...
document.addEventListener('DOMContentLoaded', () => {
  ...
  /* ---------------- 8. Placeholder link + resume link notices ---------------- */
  // These links are marked with data-placeholder-link / data-resume-link because
  // the corresponding real URL has not been provided yet. Clicking them explains
  // what to do instead of silently navigating nowhere (a broken '#').
  document.querySelectorAll('[data-placeholder-link]').forEach(link => {
    link.addEventListener('click', (e) => {
      e.preventDefault();
      window.alert('This link is a placeholder. Add the real URL in index.html (look for the matching [ADD ...] comment) to activate it.');
    });
  });
  ...
  document.querySelectorAll('[data-resume-link]').forEach(link => {
    link.addEventListener('click', (e) => {
      e.preventDefault();
      window.alert('Resume link not added yet. Upload your resume PDF to the project (e.g. /assets/resume.pdf) and update the [ADD RESUME FILE/LINK] placeholder in index.html.');
    });
  });
  ...
  form?.addEventListener('submit', (e) => {
    e.preventDefault();
    formNote.textContent = 'This is a static site with no backend attached yet, so the message above was not actually sent. Connect this form to a service like Formspree, or add [ADD EMAIL] as a mailto fallback — see the deployment guide.';
